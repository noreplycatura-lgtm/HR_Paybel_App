...
import { Download, Loader2, FileBarChart, Users, CalendarDays, Wallet, ClipboardList, FileText } from "lucide-react";
...
function StatCard({ title, value, icon: Icon, color, subtitle }: {
  ...
  const colorClasses: Record<string, { bg: string; icon: string; text: string }> = {
    ...
    indigo: { bg: 'bg-indigo-50 border-indigo-200', icon: 'text-indigo-600 bg-indigo-100', text: 'text-indigo-700' },
  };
...
export default function ReportsPage() {
  ...
  React.useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        toast({ title: "Error loading initial data", description: "Could not load some stored data.", variant: "destructive" });
    ...
    try {
      while (isBefore(currentMonthIterator, endOfMonth(endDate)) || isEqual(currentMonthIterator, startOfMonth(endDate))) {
        ...
        if (employeeDOJ && isAfter(employeeDOJ, endOfMonth(currentMonthIterator))) {
          if (isEqual(currentMonthIterator, startOfMonth(endDate))) break;
      ...
      if (csvRows.length <= minExpectedHeaderRows) {
        toast({ title: "No Data", description: `No monthly data found for ${employee.name} in the selected period.`, variant: "destructive", duration: 7000 });
      } else {
      ...
      toast({ title: "Report Generation Error", variant: "destructive" });
    } finally {
  ...
  const handleGenerateLeaveUsageReport = () => {
    ...
    if (!employee || !employee.doj || !isValid(parseISO(employee.doj))) {
      toast({ title: "Employee Data Error", variant: "destructive" });
      setIsGeneratingReport(false);
      return;
    }
    ...
    try {
      ...
      if (csvRows.length <= 1) {
        toast({ title: "No Data", description: `No leave usage data found for ${employee.name}.`, variant: "destructive" });
        setIsGeneratingReport(false);
        return;
      }
  ...
  const handleGenerateSalaryLedger = () => {
    ...
    while (isBefore(currentLoopDate, toDate) || isEqual(currentLoopDate, toDate)) {
      ...
      currentEmployeeMasterListForLedger.forEach(emp => {
        ...
        if (empDOJ && isAfter(empDOJ, endOfMonth(currentLoopDate))) return;
        if (empDOR && isBefore(empDOR, startOfMonth(currentLoopDate))) return;
        if (!empAttendanceRecord && emp.status === "Active") return;
    ...
    if (allSalarySheetDataForPeriod.length === 0) {
      toast({ title: "No Data", description: "No salary data found for any employee.", variant: "destructive" });
      setIsGeneratingReport(false);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-blue-600 to-indigo-800 p-6 text-white shadow-xl">
        ...
        <div className="relative">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-indigo-100 text-sm">Generate comprehensive HR and payroll reports</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={employeeMasterList.length}
          icon={Users}
          color="blue"
          subtitle="In master list"
        />
        <StatCard
          title="Active Employees"
          value={employeeMasterList.filter(e => e.status === "Active").length}
          icon={Users}
          color="green"
        ...
        <Card className="shadow-lg border-0">
          <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-t-lg border-b">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base">Comprehensive Employee Report</CardTitle>
                <CardDescription>Attendance, salary, and leave details for an employee</CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                ...
                <Select value={selectedEmpComp} onValueChange={handleEmployeeSelectionForCompReport}>
                  <SelectTrigger className="border-gray-300"><SelectValue placeholder="Select Employee" /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="dateFromComp" className="text-sm font-medium mb-2 block">Date From</Label>
                <Input type="date" id="dateFromComp" value={dateFromComp} onChange={e => setDateFromComp(e.target.value)} className="border-gray-300" />
              </div>
              <div>
                <Label htmlFor="dateToComp" className="text-sm font-medium mb-2 block">Date To</Label>
                <Input type="date" id="dateToComp" value={dateToComp} onChange={e => setDateToComp(e.target.value)} className="border-gray-300" />
              </div>
            </div>
            <Button onClick={handleGenerateComprehensiveReport} disabled={isGeneratingReport || !selectedEmpComp || !dateFromComp || !dateToComp} className="bg-blue-600 hover:bg-blue-700">
        ...
        <Card className="shadow-lg border-0">
          <CardHeader className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-t-lg border-b">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base">Employee Leave Usage Report</CardTitle>
                <CardDescription>Monthly leave usage and balance from joining to current date</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="pt-6 space-y-4">
            ...
            <Button onClick={handleGenerateLeaveUsageReport} disabled={isGeneratingReport || !selectedEmpLeave} className="bg-purple-600 hover:bg-purple-700">
              {isGeneratingReport ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}
        ...
        <Card className="shadow-lg border-0">
          <CardHeader className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-t-lg border-b">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base">Monthly Salary Ledger</CardTitle>
                <CardDescription>Detailed salary ledger for all employees for a selected date range</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="pt-6 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <Label htmlFor="fromMonthLedger" className="text-sm font-medium mb-2 block">From Month</Label>
                <Select value={fromMonthLedger} onValueChange={setFromMonthLedger}>
                  <SelectTrigger id="fromMonthLedger" className="border-gray-300"><SelectValue placeholder="Select Month" /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="fromYearLedger" className="text-sm font-medium mb-2 block">From Year</Label>
                <Select value={fromYearLedger > 0 ? fromYearLedger.toString() : ""} onValueChange={(value) => setFromYearLedger(parseInt(value))}>
                  <SelectTrigger id="fromYearLedger" className="border-gray-300"><SelectValue placeholder="Select Year" /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="toMonthLedger" className="text-sm font-medium mb-2 block">To Month</Label>
                <Select value={toMonthLedger} onValueChange={setToMonthLedger}>
                  <SelectTrigger id="toMonthLedger" className="border-gray-300"><SelectValue placeholder="Select Month" /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                <Label htmlFor="toYearLedger" className="text-sm font-medium mb-2 block">To Year</Label>
                <Select value={toYearLedger > 0 ? toYearLedger.toString() : ""} onValueChange={(value) => setToYearLedger(parseInt(value))}>
                  <SelectTrigger id="toYearLedger" className="border-gray-300"><SelectValue placeholder="Select Year" /></SelectTrigger>
                  <SelectContent>
